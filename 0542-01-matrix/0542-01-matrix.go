...
func updateMatrix(mat [][]int) [][]int {
    ...
    for i := 0; i < m; i++ {
        for j := 0; j < n; j++ {
            ...
            // why Inf-10 here ?
            // because at position 0,0, our minVal will stay Inf..
            // adding 1 to Inf will make it -Inf ( i.e smallest possible number )
            // which will never change and our final output will have -Inf becuase there will be no number smaller than -Inf
            // so whenever we have +Inf and there is a chance it will stay +Inf, careful adding numbers to that because adding 1 to +Inf will make it -Inf
