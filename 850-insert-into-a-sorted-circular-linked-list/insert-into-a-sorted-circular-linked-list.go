...
/*
    ...
    - there are 3 places we would want to insert x at
        ...
        2. after the ending node / before the starting node
        ...
        - and this curr node is the ending node
        - and next node is automatically starting node
        - we can insert x after ending node if x >= ending node
        - OR
        - we can insert x before starting node if x <= starting node
        ...
        - 3->4->5->1-
        - x = 2
        - entry node = 3; ending node= 5; starting node = 1
        - 2 does not fit between 3->4, 4->5 or 5->1(ending->starting)
        - 2 fits BEFORE entry node
